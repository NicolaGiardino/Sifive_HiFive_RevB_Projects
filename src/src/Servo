...
int MoveServo(struct Servo *servo, int pos)
{
//The PIN used here is the arduino relative PIN, for semplicity
	...
	unsigned int gpio;
	gpio = variant_pin_map[servo->pin].bit_pos;
	...
	switch(variant_pin_map[servo->pin].pwm_num)
	{
	...
	case 1:
		...
		switch(variant_pin_map[servo->pin].pwm_cmp_num)
		{
			case 0:
...
int EndServo(struct Servo *servo)
{
	...
	unsigned int pin = variant_pin_map[servo->pin].bit_pos;
